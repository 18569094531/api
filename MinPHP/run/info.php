...
<?php
   ...
   if($op == 'add'){
        if($type == 'do'){
            ...
            $img='';
            if ( (($_FILES["img"]["type"] == "image/png") || ($_FILES["img"]["type"] == "image/jpeg") || ($_FILES["img"]["type"] == "image/pjpeg")) && ($_FILES["img"]["size"] < 20000000))
            {
            ...
            `lastuid`,`isdel`,`memo`,`ord`,`img`
            )values (
            '{$aid}','{$num}','{$name}','{$des}','{$url}',
            '{$type}','{$parameter}','{$re}','{$lasttime}',
            '{$lastuid}','{$isdel}','{$memo}','99999','{$img}'
            )";
            $re = insert($sql);
            if($re){
   ...
   //此分类下的接口列表
      ...
      exit();
        ...
        $sql = "select api.id,aid,num,url,name,des,parameter,memo,re,lasttime,lastuid,type,login_name,img
        from api
        left join user
    ...
    <div style="border:1px solid #ddd">
        <div style="background:#f5f5f5;padding:20px;position:relative">
            ...
            <div style="margin-left:20px;">
                <form action="?act=api&tag=<?php echo $_GET['tag']?>&type=do&op=add" method="post" enctype="multipart/form-data">
                    <h5>基本信息</h5>
...
<?php }else if($op == 'edit'){ ?>
    ...
    <div style="border:1px solid #ddd">
        <div style="background:#f5f5f5;padding:20px;position:relative">
            ...
            <div style="margin-left:20px;">
                <form action="?act=api&tag=<?php echo $_GET['tag']?>&type=do&op=edit" method="post" enctype="multipart/form-data">
                    <h5>基本信息</h5>
                    <div class="form-group has-error">
                        <div class="input-group">
                            <div class="input-group-addon">接口编号</div>
        ...
        <div class="info_api" style="border:1px solid #ddd;margin-bottom:20px;" id="info_api_<?php echo md5($v['id'])?>">
            <div style="background:#f5f5f5;padding:20px;position:relative">
                ...
                <div>
                    ...
                    ?>
                    <kbd style="color:<?php echo $color?>"><?php echo $v['type']?></kbd> - <a target="_blank" href="<?php echo $v['url']?>"><kbd><?php echo $v['url']?></kbd></a>
                </div>
            </div>
            <?php if(!empty($v['des'])){ ?>
            <div class="info">
            ...
            <?php } ?>
             ...
             <?php if(!empty($v['img'])){ ?>
            <div style="background:#ffffff;padding:20px; ">
                <h5 class="textshadow" >图片</h5>
                <img style="width:20%;" src="<?php echo $v['img']; ?>"/> 
            </div>
            <?php } ?>
        </div>
        <!--接口详细列表end-->
